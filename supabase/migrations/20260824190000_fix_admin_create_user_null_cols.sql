-- ============================================================
-- 修复：手动插入 auth.users 时漏列导致 NULL，
--       GoTrue 登录时 Scan 到 NULL 报 500 "Database error querying schema"
-- 修复：INSERT 补齐 GoTrue 必填列（对照 GoTrue 正常创建的用户值），
--       避免新账号再出现 NULL 列。
-- 已存在的账号已用 UPDATE 修复（confirmation_token/recovery_token/
--       email_change_token_new/email_change/raw_app_meta_data 补空串/默认值）。
-- ============================================================
